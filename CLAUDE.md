...
- **Risk formula**: `0.35*churn + 0.25*coupling + 0.15*coverage_gap + 0.10*coverage_depth + 0.10*author_concentration + 0.05*test_instability + hidden_risk_factor + new_file_boost`. The first 6 components are reweighted when 2+ are uniform or when any zero-valued uniform component is detected (provably absent data). `hidden_risk_factor` (0–0.15) is added separately from dynamic/eval import edge density. `new_file_boost` is 0.5 for files with zero churn and zero test coverage, ensuring new/untracked files surface in rankings. Coupling uses `max(git co-change, static import-graph)` breadth. `coverage_gap` is graduated (quantized to 0.05 steps via 20-step quantization: 0.0, 0.05, 0.10, ..., 1.0). `coverage_depth = min(distinct_covering_tests/5, 1.0)`. `get_risk_map` may reweight the composite when uniform components are detected. `proximity_adjustment` reduces `coverage_gap` by import distance to tested code for any file with gap > 0.0 (not just completely untested files). `risk_map` accepts `working_tree=true` to include untracked files.
