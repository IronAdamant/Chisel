...
- **Risk formula**: `0.35*churn + 0.25*coupling + 0.15*coverage_gap + 0.10*coverage_depth + 0.10*author_concentration + 0.05*test_instability + hidden_risk_factor + new_file_boost`. The first 6 components are reweighted when 2+ are uniform or when any zero-valued uniform component is detected (provably absent data). `hidden_risk_factor` (0–0.15) is added separately from dynamic/eval import edge density. `new_file_boost` is 0.5 for files with zero churn and zero test coverage, ensuring new/untracked files surface in rankings. Pass `exclude_new_file_boost=true` to `risk_map` or `triage` to suppress this for stable long-term audits. Coupling uses first-class hybrid scoring: `max(cochange, import, 0.5*cochange + 0.5*import)`. `coverage_gap` is graduated (quantized to 0.05 steps via 20-step quantization: 0.0, 0.05, 0.10, ..., 1.0). `coverage_depth = min(distinct_covering_tests/5, 1.0)`. `get_risk_map` may reweight the composite when uniform components are detected. `proximity_adjustment` reduces `coverage_gap` by import distance to tested code for any file with gap > 0.0 (not just completely untested files). `risk_map` defaults to `coverage_mode="line"` and `proximity_adjustment=True`, and accepts `working_tree=true` to include untracked files.
- **Co-change ingest**: `compute_co_changes` uses adaptive `min_count` from `coupling_threshold()`; queries use `meta.co_change_query_min` so stored pairs are visible. **Single-author detection**: when all commits have one distinct author, the adaptive threshold is halved (`max(1, threshold // 2)`) so that a solo developer's commit patterns still surface coupling signal instead of returning 0.0 for all file pairs. `distinct_authors` count is stored in meta. Branch-only pairs stored in `branch_co_changes` from `merge-base..HEAD`. Commits touching >50 files are skipped (bulk operations).
...
- **gitignore-aware scanning** (v0.13): `git_visible_paths()` in `project.py` returns the set of tracked + untracked-but-not-ignored files (`git ls-files --cached --others --exclude-standard`); `_scan_code_files()` and `TestMapper.discover_test_files()` filter through it and prune ignored trees from the walk entirely (`prune_walk_dirs`). Untracked files stay visible (working-tree analysis unaffected); ignored fixture/vendor trees are invisible. `CHISEL_INCLUDE_IGNORED=1` disables; non-git projects are unfiltered. Tests: `tests/test_gitignore_filter.py`.
- **Edge-build memoization** (v0.13): `build_test_edges()` memoizes import-path matching per module path and dependency extraction per test file — matching depends only on (module_path, code file), never the test unit. A/B-verified byte-identical output, ~76× faster. `update()` skips the edge rebuild phases entirely when no files changed and no new commits (`edge_rebuild_skipped: true`) and restricts per-function `git log -L` churn to changed files (`unit_churn_files` param on `_compute_churn_and_coupling`).
- **CLI exit codes**: console script entry point is `cli_entry()` (NOT `main()`): `main()` returns the result dict for programmatic/test use, `cli_entry()` converts to exit codes (`error`/`git_error` → 1, else 0). `sys.exit(<dict>)` would exit 1 and dump the dict to stderr.
...
- **`suggest_tests`**: Returns impacted tests ranked by relevance. Auto-fallbacks to stem-matching across all known test files when no DB or static edges exist. Accepts `fallback_to_all=True` to force fallback mode. Accepts `working_tree=True` to also scan untracked files on disk. Output is capped at `_WORKING_TREE_SUGGEST_LIMIT` (30) entries when `working_tree=True` to prevent output explosion in large projects.
...
- **Coverage gap modes**: `risk_map` defaults to `coverage_mode="line"` (weighted by line count so large untested units have proportionally higher gap); pass `"unit"` for equal weight per code unit. `_quantize_gap` uses 20 steps (0.05 increments) for fine-grained coverage differentiation. `proximity_adjustment` defaults to True and applies to any file with `coverage_gap > 0.0` (not just completely untested files), giving partial credit to files imported by tested code.
