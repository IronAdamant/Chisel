...
class ImpactAnalyzer:
        ...
        for file_path in changed_files:
            ...
            for cu in code_units:
                ...
                for edge in edges:
                    ...
                    if test and test["id"] not in impacted:
                        impacted[test["id"]] = {
                            "test_id": test["id"],
    ...
    def who_reviews(self, file_path):
        """Suggest reviewers based on blame ownership.

        Returns:
            List of dicts: {author, author_email, line_count, percentage}
        """
        ...
        for block in blame_data:
            ...
            author_lines[block["author"]]["line_count"] += n
            author_lines[block["author"]]["email"] = block.get("author_email", "")
            total += n

        result = []
        for author, info in author_lines.items():
            pct = (info["line_count"] / total * 100) if total > 0 else 0
            result.append({
                "author": author,
